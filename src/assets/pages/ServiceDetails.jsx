...
const ServiceDetails = () => {
  ...
  const loginuseremail = user?.email || 'email';

  const { id } = useParams();
  const navigate = useNavigate();
  const { state } = useLocation();
  const [service, setService] = useState(state ? state.service : null);
  ...
  const [userData, setUserData] = useState({
    ...
    userEmail: loginuseremail, // Example, you can replace with actual user data
  ...
  const handleBookNow = () => {
    setIsModalOpen(true); // Open the modal when the "Book Now" button is clicked
  };
  ...
  const handlePurchase = () => {
    ...
    console.log(serviceToAdd);  // Ensure proemail is included in the payload
